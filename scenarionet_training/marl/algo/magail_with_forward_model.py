...
class Discriminator(TorchModelV2, nn.Module):
    def __init__(
        ...
        # === Forward Model ===
        layers = []
        prev_layer_size = VEHICLE_STATE_DIM + int(np.product(action_space.shape)) + DYNAMICS_PARAMETERS_DIM
        for size in [256, 256]:
            layers.append(
                SlimFC(
                    in_size=prev_layer_size,
        ...
        # TODO: Haven't times 2 yet. But in future we need to introduce log std here.
    ...
    def get_dynamics_parameters_distribution(self):
        # TODO: Haven't times 2 yet. But in future we need to introduce log std here.
        return self.dynamics_parameters, torch.nn.Parameter(torch.zeros([DYNAMICS_PARAMETERS_DIM]) - 100, requires_grad=True)
    ...
    def compute_forward_loss(self, *, state, next_state, action, dynamics_parameters):
        ...
        forward_loss = self._forward_model_loss(input=pred_next_state, target=next_state)
        return forward_loss

    def compute_dynamics_loss(self, state, action):
        # TODO: add a flag to support raw obs and raw state

        # Sample from current distribution
        # TODO: We don't have STD for the distribution yet.
        ...
        ret = self.get_reward(state=state, next_state=fake_next_obs)
...
class NewPolicy(CCPPOPolicy):
    ...
    def update_discriminator(self, train_batch):
        discriminator_loss, reward = self.discriminator.compute_discriminator_loss(
    ...
    def update_forward_model(self, train_batch):
        ...
        forward_loss = self.discriminator.compute_forward_loss(
        ...
        # TODO: Fill dynamics_parameters, state, next_state into train_batch! Do this in postprocess_trajectory
    ...
    def update_dynamics(self, train_batch):
        clipped_actions = np.clip(train_batch["actions"], self.action_space.low[0], self.action_space.high[0])
        dynamics_loss = self.discriminator.compute_dynamics_loss(
            state=convert_to_torch_tensor(train_batch[SampleBatch.OBS], device=self.device),
            # next_state=convert_to_torch_tensor(train_batch[SampleBatch.NEXT_OBS], device=self.device),
            action=convert_to_torch_tensor(clipped_actions, device=self.device),
            # dynamics_parameters=convert_to_torch_tensor(train_batch["dynamics_parameters"], device=self.device)
    ...
    def training_step(self):
        ...
        for i in range(self.config["discriminator_num_iters"]):
            ...
            print("===== Discriminator Training {} Iterations ===== Stats: {}".format(
                i, {k: round(sum(v) / len(v), 4) for k, v in recorder.items() if "discriminator" in k}
            ))

        # Update Forward Model
        # TODO: split the hypers
        for i in range(self.config["discriminator_num_iters"]):
        ...
        def _set_dynamics(e):
            e.set_dynamics_parameters_distribution(dynamics_parameters_mean=d_mean, dynamics_parameters_std=d_std)
...
def _test():
    ...
    config = dict(
        ...
        env_config=dict(
            ...
            # TODO: We haven't implement discrete GAIL yet!
