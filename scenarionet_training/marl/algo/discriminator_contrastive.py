...
class CoPOModelWithForward(CoPOModel):
    def __init__(
        ...
        layers.append(SlimFC(
            in_size=prev_layer_size,
            out_size=forward_model_latent_dim,  # Output a state embedding!
            initializer=normc_initializer(0.01),
            activation_fn=None
        ))
        self._forward_model_key = nn.Sequential(*layers)

    def get_similarity(self, state, next_state):
        assert isinstance(state, torch.Tensor)
        assert state.dim() == 2
        assert next_state.dim() == 2

        norm_t = torch.nn.functional.normalize(state)
        norm_tp1 = torch.nn.functional.normalize(next_state)
        dot = norm_t.T @ norm_tp1
        return dot
