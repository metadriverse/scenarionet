...
from ray.rllib.models.torch.torch_distributions import TorchDiagGaussian as TorchDiagGaussianNative
...
FORWARD_MODEL_LOGP = "forward_model_logp"
    ...
    def __init__(self, algo_class=None):
        ...
        self.update_from_dict({
            "model": {
                ...
                "custom_model": "copo_model_with_forward",
            }})

        # ===== New Discriminator Config =====
        self.discriminator_l2 = 1e-5
        self.discriminator_lr = 5e-4
        self.discriminator_num_iters = 30
        self.discriminator_sgd_minibatch_size = 512
        self.discriminator_entropy_coeff = 0.0
...
# ========== New Model ==========
class CoPOModelWithForward(CoPOModel):
    def __init__(
            self, obs_space: gym.spaces.Space, action_space: gym.spaces.Space, num_outputs: int,
            model_config: ModelConfigDict, name: str
    ):
        super(CoPOModelWithForward, self).__init__(
            obs_space, action_space, num_outputs, model_config, name
        )

        activation = model_config.get("fcnet_activation")
        forward_model_latent_dim = int(np.product(obs_space.shape))
        # Part 1: Encoder
        layers = []
        ...
        self._forward_model_key = nn.Sequential(*layers)
        self._forward_model_loss = torch.nn.GaussianNLLLoss()

    # def parameters(self):
    #     ret = super(CoPOModelWithForward, self).parameters()
    #     return ret

    def get_transition_logits(self, obs):
        ret = self._forward_model_key(obs)
        mean, log_std = torch.chunk(ret, 2, dim=1)
        log_std = log_std.clamp(-20, 10)
        return torch.cat([mean, log_std], dim=1)



    def compute_transition_logps(self, obs, next_obs):
        latent_logits = self.get_transition_logits(obs)
        dist = TorchDiagGaussian(inputs=latent_logits, model=self)
        query = next_obs
        return dist.logp(query)

ModelCatalog.register_custom_model("copo_model_with_forward", CoPOModelWithForward)
...
class Discriminator(TorchModelV2, nn.Module):
    def __init__(
        ...
        # layers = []
        # # prev_layer_size = int(np.product(obs_space.shape)) * 2
        ...
        # layers.append(SlimFC(
        #     in_size=prev_layer_size,
        #     out_size=forward_model_latent_dim,
        #     initializer=normc_initializer(0.01),
        #     activation_fn=None
        # ))
        # self._forward_model_query = nn.Sequential(*layers)
        # import torch
        # self._forward_model_recon_loss = torch.nn.GaussianNLLLoss
    ...
    def get_forward_parameters(self):
        raise ValueError()
        # return list(self._forward_model_key.parameters()) + list(self._forward_model_query.parameters())
        return list(self._forward_model_key.parameters())
    ...
    def get_reward(self, *, obs, act=None, next_obs=None, action_logp=None, discriminator_reward_native=False,
        ...
        log_p_tau = log_p_tau.clamp(-5, 2).reshape(-1)
        log_q_tau = log_q_tau.clamp(-5, 2).reshape(-1)
    ...
    def compute_inverse_loss(self, obs, next_obs, actions, low, high):
        ...
        inv_loss = self._inverse_loss(input=pseudo_action, target=actions)
    ...
    def compute_transition_logps(self, obs, next_obs):
        raise ValueError()
        latent_logits = self._forward_model_key(obs)
        dist = TorchDiagGaussian(inputs=latent_logits, model=self)
        # latent_mean, latent_log_std = torch.chunk(latent_logits, 2, dim=1)
        query = next_obs
        # latent_std = torch.exp(latent_log_std.clamp(-20, 10)) # IS STD!
        # dist = TorchDiagGaussianNative(loc=latent_mean, scale=latent_std)
        return dist.logp(query)

    def compute_forward_loss(self, obs, next_obs, entropy_coeff=0.0):
        raise ValueError()
        # Credit: https://github.com/AntixK/PyTorch-VAE/blob/master/models/cvae.py

        # Get latent space distribution
        latent_logits = self._forward_model_key(obs)
        latent_mean, latent_log_std = torch.chunk(latent_logits, 2, dim=1)
        latent_var = torch.square(torch.exp(latent_log_std.clamp(-5, 2)))

        # Shapes: [bs, latent dim], [bs,]

        # Get the query latent
        # query = self._forward_model_query(next_obs)

        query = next_obs

        nll_loss = self._forward_model_loss(input=latent_mean, target=query, var=latent_var)

        mse = torch.nn.functional.mse_loss(input=latent_mean, target=query).mean().item()

        entropy = TorchDiagGaussianNative(loc=latent_mean, scale=torch.sqrt(latent_var)).entropy()
        entropy = entropy.mean()
        loss = nll_loss - entropy * entropy_coeff
    ...
    def compute_discriminator_loss_old(self, *, obs, act, next_obs, action_logp, labels, gamma, add_action):
        raise ValueError()
        log_q_tau = action_logp
    ...
    def compute_discriminator_loss_new(self, *, obs, act, next_obs, transition_logps, labels, gamma, add_action):

        log_q_tau = transition_logps
        ...
        log_p_tau = log_p_tau.reshape(-1)
        # log_q_tau = log_q_tau.clamp(-20, 10).reshape(-1)
        log_q_tau = log_q_tau.reshape(-1)

        prob_from_policy = torch.exp(log_q_tau)

        sum_and_exp = torch.exp(log_p_tau) + prob_from_policy
        ...
        return loss, reward, value, prob_from_policy
...
class NewPolicy(CCPPOPolicy):
    def __init__(self, observation_space, action_space, config):
        ...
        #     lr=self.config["discriminator_lr"],  # Just reuse the hyperparameters!!!
        ...
        self.view_requirements[SampleBatch.ACTIONS] = ViewRequirement(
            space=Box(action_space.low[0], action_space.high[0], shape=(int(np.product(action_space.shape)),))
        )
    ...
    def update_forward_model(self, train_batch):
        raise ValueError()
        # clipped_actions = np.clip(train_batch["actions"], self.action_space.low[0], self.action_space.high[0])
        forward_loss, stats = self.model.compute_forward_loss(
            obs=convert_to_torch_tensor(train_batch[SampleBatch.OBS], device=self.device),
            next_obs=convert_to_torch_tensor(train_batch[SampleBatch.NEXT_OBS], device=self.device),
            # actions=convert_to_torch_tensor(clipped_actions, device=self.device),
            # low=self.action_space.low[0],
            # high=self.action_space.high[0]
            entropy_coeff=self.config["discriminator_entropy_coeff"]
        )
        ...
        stats = {
            ...
            "forward_mean": latent_mean.mean().item(),
            "forward_var": latent_var.mean().item(),
            "forward_mse": stats["mse"],
            "forward_nll_loss": stats["nll_loss"].item(),
            "forward_entropy": stats["entropy"].item(),
            # "forward_abs_mean": np.mean(np.abs(l1_diff)),
            # "forward_abs_dim0_mean": np.mean(np.abs(l1_diff)[:, 0]),
            # "forward_abs_dim1_mean": np.mean(np.abs(l1_diff)[:, 1]),
            # "forward_l1_diff_max": np.max(l1_diff),
            # "forward_l1_diff_min": np.min(l1_diff),
            # "forward_l1_diff_mean": np.mean(l1_diff),
            # "forward_pseudo_action_mean": pseudo_action.mean().item(),
            # "forward_pseudo_action_min": pseudo_action.min().item(),
            # "forward_pseudo_action_max": pseudo_action.max().item(),
        }
    ...
    def update_discriminator(self, train_batch):
        # train_batch = self._lazy_tensor_dict(train_batch, device=self.device)
        #
        # discriminator_loss, reward, value = self.discriminator.compute_discriminator_loss_old(
        #     obs=convert_to_torch_tensor(train_batch[SampleBatch.OBS], device=self.device),
        ...
        assert self.config["discriminator_add_action"] is False

        discriminator_loss, reward, value, prob_from_policy = self.discriminator.compute_discriminator_loss_new(
            obs=convert_to_torch_tensor(train_batch[SampleBatch.OBS], device=self.device),
            # act=convert_to_torch_tensor(train_batch[SampleBatch.ACTIONS], device=self.device),
            act=None,
            next_obs=convert_to_torch_tensor(train_batch[SampleBatch.NEXT_OBS], device=self.device),
            transition_logps=convert_to_torch_tensor(train_batch["transition_logps"], device=self.device),
            labels=convert_to_torch_tensor(train_batch["labels"], device=self.device),
            gamma=self.config["gamma"],
            add_action=self.config["discriminator_add_action"]
        )
        ...
        stats = {
            ...
            "discriminator_prob_from_policy": prob_from_policy.mean().item(),
            "discriminator_log_prob_from_policy": train_batch["transition_logps"].mean().item(),
        }
    ...
    def loss(self, model, dist_class, train_batch):
        ...
        # ========== PPO Loss for training policy network ==========
        logp_ratio = torch.exp(
        ...
        # ========== OUR NEW LOSS to train the FORWARD MODEL ==========


        # Get latent space distribution
        # if not hasattr(self, "discriminator"):
        #     # The policy is not fully initialized yet!
        #     forward_mean_policy_loss = torch.zeros(())
        #     forward_curr_entropy = torch.zeros(())
        #     forward_surrogate_loss = torch.zeros(())
        #     forward_mean_kl_loss = torch.zeros(())
        #     forward_mean_entropy = torch.zeros(())
        # else:
        forward_logits = self.model.get_transition_logits(train_batch["obs"])
        forward_curr_action_dist = TorchDiagGaussian(forward_logits, model)


        forward_logp_new = forward_curr_action_dist.logp(train_batch[SampleBatch.NEXT_OBS])
        forward_logp_old = train_batch[FORWARD_MODEL_LOGP]
        forward_ratio_diff = forward_logp_new - forward_logp_old
        # forward_ratio_diff = forward_ratio_diff.clamp(-20, 10)
        forward_logp_ratio = torch.exp(forward_ratio_diff)
        forward_logp_ratio = torch.nan_to_num(forward_logp_ratio, nan=0.0)

        # PZH: Just ignore this!!
        # if self.config["kl_coeff"] > 0.0:
        #     forward_prev_action_dist = dist_class(train_batch[SampleBatch.ACTION_DIST_INPUTS], model)
        #     forward_action_kl = forward_prev_action_dist.kl(forward_curr_action_dist)
        #     forward_mean_kl_loss = reduce_mean_valid(forward_action_kl)
        # else:
        forward_mean_kl_loss = torch.tensor(0.0, device=logp_ratio.device)

        forward_curr_entropy = forward_curr_action_dist.entropy()
        forward_mean_entropy = reduce_mean_valid(forward_curr_entropy)

        # Modification: The advantage is changed here!
        # Note: the advantages here is already modified by the coordinated advantage!
        advantages = train_batch["normalized_advantages"]
        # forward_surrogate_loss = torch.min(
        #     advantages * forward_logp_ratio,
        #     advantages * torch.clamp(
        #         forward_logp_ratio, 1 - self.config["clip_param"], 1 + self.config["clip_param"]
        #     )
        # )
        ...
        forward_mean_policy_loss = reduce_mean_valid(-forward_surrogate_loss)

        # ========== PPO Loss for training Value / Advantage ==========
        ...
        total_loss = reduce_mean_valid(
            ...
            # ========== OUR NEW LOSS!!! ==========
            - forward_surrogate_loss
            - self.entropy_coeff * forward_curr_entropy
        )
        ...
        if self.config["kl_coeff"] > 0.0:
            ...
            total_loss += self.kl_coeff * forward_mean_kl_loss
        ...
        model.tower_stats["policy_ratio"] = logp_ratio.mean()

        # ========== OUR NEW ==========
        model.tower_stats["forward_mean_policy_loss"] = forward_mean_policy_loss
        model.tower_stats["forward_mean_entropy"] = forward_mean_entropy
        model.tower_stats["forward_mean_kl_loss"] = forward_mean_kl_loss
        model.tower_stats["forward_ratio"] = forward_logp_ratio.mean()
        model.tower_stats["forward_model_logp_new"] = forward_logp_new.mean()
    ...
    def stats_fn(self, train_batch):
        ...
        for k in [
            ...
            "forward_mean_kl_loss", "forward_mean_entropy", "forward_mean_policy_loss",
            "forward_ratio", "policy_ratio", "forward_model_logp_new"
        ]:
        ...
        for k in [FORWARD_MODEL_LOGP, "step_lcf", GLOBAL_ADVANTAGES, NEI_ADVANTAGE, SampleBatch.ACTION_LOGP]:
    ...
    def postprocess_trajectory(self, sample_batch, other_agent_batches=None, episode=None):
        ...
        with torch.no_grad():
            ...
            if episode is None:
                ...
                sample_batch[FORWARD_MODEL_LOGP] = np.zeros_like(
                    sample_batch[SampleBatch.REWARDS],
                    dtype=sample_batch[SampleBatch.REWARDS].dtype
                )
            ...
            if episode is not None:  # After initialization
                ...
                sample_batch[FORWARD_MODEL_LOGP] = convert_to_numpy(self.model.compute_transition_logps(
                    obs=convert_to_torch_tensor(sample_batch[SampleBatch.OBS], device=self.device),
                    next_obs=convert_to_torch_tensor(sample_batch[SampleBatch.NEXT_OBS], device=self.device),
                ))
    ...
    # def refill_training_batch(self, train_batch):
    #     assert len(train_batch.policy_batches) == 1
    #     before_count = train_batch.count
    #
    #     # process batch transform it to torch
    #     batch = train_batch.policy_batches["default"]
    #     batch = self._lazy_tensor_dict(batch, device=self.device)
    #
    #     # compute (fake) reward
    #     reward = self.discriminator.get_reward(obs=batch[SampleBatch.OBS])
    #     batch[SampleBatch.REWARDS] = reward
    #
    #     # Compute advantage: neighbor adv, global adv, ego adv.
    #
    #     dddd = batch.split_by_episode()
    #
    #     print(1111)
    ...
    #     # )
    #
    #     # assert before_count == batch.count
    #     ret = MultiAgentBatch({"default": batch}, batch.count)
    #     return train_batch
...
class NewTrainer(CCPPOTrainer):
    ...
    def generate_mixed_batch_for_discriminator(self, train_batch, policy):
        ...
        data_size = train_batch.policy_batches["default"].count
        ...
        native_obs_len = expert_batch["obs"].shape[1]

        assert train_batch.policy_batches["default"][SampleBatch.OBS].shape[1] == native_obs_len
        ...
        assert expert_batch["obs"].shape[1] == agent_batch["obs"].shape[1]
        ...
        with torch.no_grad():
            dis_batch["transition_logps"] = policy.model.compute_transition_logps(
                obs=convert_to_torch_tensor(dis_batch[SampleBatch.OBS], device=policy.device),
                next_obs=convert_to_torch_tensor(dis_batch[SampleBatch.NEXT_OBS], device=policy.device),
            )
    ...
    def training_step(self):
        ...
        #     assert self.config["discriminator_sgd_minibatch_size"] is not None
        #     for mb_id, minibatch in enumerate(
        #             minibatches(train_batch.policy_batches["default"],
        #                         self.config["discriminator_sgd_minibatch_size"])):
        ...
        # Train the forward dynamic model first!
        assert len(train_batch.policy_batches) == 1
        recorder = defaultdict(list)
        # for i in range(self.config["discriminator_num_iters"]):
        #     assert self.config["discriminator_sgd_minibatch_size"] is not None
...
def _test():
    ...
    config = dict(
        ...
        discriminator_entropy_coeff=7e-4
    )
    results = train(
        NewTrainer,
