...
class DiscriminatorDiscrete(TorchModelV2, nn.Module):
    def __init__(
        ...
        layers = []
        prev_layer_size = 269 * 2  # Hardcoded!!!
    ...
    def compute_dynamics_loss(self, state, action, action_logp, add_action, gamma, device=None):
        ...
        # Call very complex internal mechanism to get the discriminator prediction (scores in [0, 1])
        rew = self.get_internal_reward(obs=state, act=action, add_action=add_action)
        next_value = self.get_value(fake_next_obs)
        ...
        # print(
        #     "Dy min {}, Dy max {}, Fake next obs min {}, max {}. Scores min {}, max {}, shape {}, labels shape {}, action min {}, action max {}".format(
        #         dynamics_parameters.min(), dynamics_parameters.max(), fake_next_obs.min(), fake_next_obs.max(),
        #         scores.min(), scores.max(), scores.shape, labels.shape, action.min(), action.max()
        #     ))
...
# ========== The New Trainer ==========
class MultiAgentAIRL(CCPPOTrainer):
    @classmethod
    ...
    def generate_mixed_batch_for_discriminator(self, train_batch, policy):
        ...
        agent_batch["labels"] = np.zeros([agent_batch.count, ], dtype=data_type)
